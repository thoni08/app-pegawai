...
@section('content')
  ...
  <section class="bg-white dark:bg-gray-900">
    <div class="max-w-2xl px-4 py-8 mx-auto lg:py-16">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Data Jabatan</h1>
        <p class="text-sm text-gray-600 dark:text-gray-400">Perbarui informasi jabatan sesuai kebutuhan.</p>
      </div>
      ...
      <form action="{{ route('positions.update', $position->id) }}" method="POST" class="bg-white dark:bg-gray-900">
        @csrf
        @method('PUT')
        ...
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
          @if(in_array('nama_jabatan', $positionColumns, true))
            <div class="sm:col-span-2">
              ...
              <input type="text" id="nama_jabatan" name="nama_jabatan" value="{{ old('nama_jabatan', $position->nama_jabatan) }}" required
          ...
          @if(in_array('gaji_pokok', $positionColumns, true))
            <div class="w-full">
              ...
              <input type="number" step="0.01" id="gaji_pokok" name="gaji_pokok" value="{{ old('gaji_pokok', $position->gaji_pokok) }}" required
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
              @error('gaji_pokok')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
              @enderror
