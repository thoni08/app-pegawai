  ...
  <section class="bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
      <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">Lengkapi data absensi karyawan di bawah ini.</p>
      </div>
      ...
      <form action="{{ route('attendance.store') }}" method="POST" class="bg-white dark:bg-gray-900">
        @csrf
        <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
            ...
            <div class="sm:col-span-2">
              ...
              <select id="karyawan_id" name="karyawan_id" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                ...
                @foreach($employees as $employee)
                  <option value="{{ $employee->id }}" @selected(old('karyawan_id') == $employee->id)>
          ...
          @if(in_array('tanggal', $attendanceColumns, true))
            <div class="w-full">
              ...
              <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal') }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
              @error('tanggal')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
              @enderror
            </div>
          ...
          @if(in_array('waktu_masuk', $attendanceColumns, true))
            <div class="w-full">
              ...
              <input type="time" id="waktu_masuk" name="waktu_masuk" value="{{ old('waktu_masuk') }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
              @error('waktu_masuk')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
              @enderror
            </div>
          ...
          @if(in_array('waktu_keluar', $attendanceColumns, true))
            <div class="w-full">
              ...
              <input type="time" id="waktu_keluar" name="waktu_keluar" value="{{ old('waktu_keluar') }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
              @error('waktu_keluar')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
              @enderror
            </div>
          ...
          @if(in_array('status_absensi', $attendanceColumns, true))
            <div class="w-full sm:col-span-2">
              ...
              <select id="status_absensi" name="status_absensi" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                <option value="" disabled {{ old('status_absensi') ? '' : 'selected' }}>-- Pilih Status --</option>
                @foreach($statuses as $status)
                  @php($statusLabel = ucfirst(str_replace('_', ' ', $status)))
                  <option value="{{ $status }}" @selected(old('status_absensi') == $status)>{{ $statusLabel }}</option>
                @endforeach
              </select>
