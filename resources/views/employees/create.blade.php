  ...
  <section class="bg-white dark:bg-gray-900">
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
      <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">Lengkapi data pegawai di bawah ini.</p>
      </div>
      ...
      <form action="{{ route('employees.store') }}" method="POST" class="bg-white dark:bg-gray-900">
        @csrf
          ...
          @if(in_array('alamat', $employeeColumns, true))
            <div class="sm:col-span-2">
              ...
              <textarea id="alamat" name="alamat" rows="3" required
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">{{ old('alamat') }}</textarea>
              @error('alamat')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
              @enderror
            </div>
            ...
            <div class="w-full">
              ...
              <select id="departemen_id" name="departemen_id" required
                ...
                @foreach($departments as $department)
                  <option value="{{ $department->id }}" @selected(old('departemen_id') == $department->id)>
            ...
            <div class="sm:col-span-2">
              ...
              <select id="status" name="status" required
                ...
                @foreach($statuses as $statusOption)
                  <option value="{{ $statusOption }}" @selected(old('status') == $statusOption)>{{ ucfirst($statusOption) }}</option>
                @endforeach
              </select>
              @error('status')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
